...
| 10b | [From-scratch beam-search CF generation](stages/10-beam-search-from-scratch.md) | DONE | beam_search.py (task-guided beam over raw TabPFNRegressor; per-feature λ soft-freeze; hard [0,1] rejection; validity rerank) + exp4_beam_search.py runner + test_beam_search.py (8 offline tests, no checkpoint needed). Two regimes (Decision #11): frozen-immutable (Set 1, ~Exp2 baseline) and from-scratch (Set 2). | d9e30bb (+follow-up) |
...
11. **Exp4 (Stage 10) generates *every* feature from scratch; immutables are soft-frozen, not observed.** The user's request was to generate counterfactuals fully from scratch (mask all features, condition only on `Y=target`). This drops the Exp 2/3 structural `true_actionability=1.0` guarantee, so immutables are kept near-factual via a **large per-feature proximity weight** (`lambda_immutable`, default 100) rather than freezing them as observed context. `immutable_drift` is reported as a first-class metric. Consequence: from-scratch conditioning is *richer* than Exp 2's flat imputation (feature `k` conditions on `Y` + the `k−1` already-generated features), which is why it eliminates the OOB/extrapolation that broke Exp 2.

12. **Exp4 reimplements the autoregressive chain on the raw `TabPFNRegressor`, not `TabPFNUnsupervisedModel.impute`.** Beam search needs explicit control of the per-step branch point (`predict(output_type="full")` → `criterion`/`logits` → `icdf`/`mode` candidates + `forward` log-density scoring), which `impute()` hides. This keeps Exp 4 dependency-light (no `tabpfn-extensions` needed) and lets its 8 unit tests run fully offline against a standalone `FullSupportBarDistribution`. `lambda_actionable` is the validity↔proximity dial; because TabPFN log-densities are O(several units), it must be O(20–100) to bite (λ≈1 is density-dominated). The entire frontier is plausible (LOF≈1.0, frac_oob=0.00) but caps at validity ≈0.35 < Exp 2's 0.52 — from-scratch generation **trades validity for plausibility**.
