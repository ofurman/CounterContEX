...
| 10a | Iterative greedy CF (sparsity-optimal) | PROMOTED | Promoted to its own standalone plan: [`iterative-greedy-cf`](../iterative-greedy-cf/index.md) (4 stages: greedy core + both selectors, selector ablation, kNN context selection, context size×strategy ablation). Builds on Stages 1–9 here. | — |
| 10b | [Beam-search CF generation (frozen vs from-scratch)](stages/10-beam-search-from-scratch.md) | DONE | beam_search.py (task-guided beam over raw TabPFNRegressor; hard [0,1] rejection; validity rerank; `freeze_immutable` mode) + exp4_beam_search.py (two regimes) + test_beam_search.py (9 offline tests). **Set 2 from-scratch (mask nothing)**: HELOC validity **1.0**, LOF **1.0**, frac_oob **0** — strictly beats Exp2, but immutables drift 0.12 (not actionable). **Set 1 frozen immutables (actionable)**: HELOC validity collapses to 0.13 + LOF 7.9e6 (off-manifold) despite true_action=1.0. Finding: actionability ⟂ validity+plausibility on HELOC (Decisions #11–13). MOONS validity=1.0 both. | d9e30bb (+follow-up) |

Statuses: `PENDING` -> `IN_PROGRESS` -> `DONE` | `BLOCKED` | `SKIPPED` | `PROMOTED` (moved to a standalone plan)

Phases: A = Stages 1–3 (infra), B = Stages 4–5 (experiments), C = Stage 6 (refinement), **D = Stages 7–8 (post-review remediation)**, **E = Stage 9 (feature-ordering ablation)**, **F = Stage 10b (from-scratch beam search)**. Stage 4 is a **gate** for Stage 5: if single-feature reconstruction is no better than the marginal baseline, note it prominently before proceeding — Stage 5 still runs but expectations are adjusted. Stages 9 and 10 are independent follow-up experiments; each depends only on Stages 1–8 being DONE. Stage 10b is developed on branch `lukasz/zeroshot-cf-beam-search` (worktree). The iterative-greedy work (originally also drafted as Stage 10, now 10a) was promoted to the standalone [`iterative-greedy-cf`](../iterative-greedy-cf/index.md) plan.
...
11. **Exp4 (Stage 10) is run in TWO regimes that differ only in whether immutables are masked.** The report contains both: **Set 1 — frozen immutables** (immutables *observed*/held at the factual; beam generates only actionables; `true_actionability=1.0`; the apples-to-apples comparison to the Exp 2/3 imputation baseline), and **Set 2 — from scratch** (mask nothing; every feature generated, conditioned only on `Y=target`; immutables drift, reported via `immutable_drift`). `freeze_immutable` selects the regime in `generate_cf_beam`. For MOONS (no immutables) the regimes coincide. The earlier soft-freeze default (large `lambda_immutable`) is dropped in favour of these two clean endpoints.

12. **Exp4 reimplements the autoregressive chain on the raw `TabPFNRegressor`, not `TabPFNUnsupervisedModel.impute`.** Beam search needs explicit control of the per-step branch point (`predict(output_type="full")` → `criterion`/`logits` → `icdf`/`mode` candidates + `forward` log-density scoring), which `impute()` hides. This keeps Exp 4 dependency-light (no `tabpfn-extensions` needed) and lets its 9 unit tests run fully offline against a standalone `FullSupportBarDistribution`. `lambda_actionable` is the proximity dial; because TabPFN log-densities are O(several units), it must be O(20–100) to bite (λ≈1 is density-dominated).

13. **Key Exp4 finding — actionability is in fundamental tension with validity+plausibility on HELOC.** *Correcting an earlier mis-analysis* (a transient claim that from-scratch "caps at validity 0.35" was an artifact of soft-freezing immutables to wrong-class values): **Set 2 from-scratch reaches validity 1.0, LOF 1.0, frac_oob 0** on HELOC — strictly beating Exp 2 — by regenerating *all* features (the conditioning is *richer*, not sparser: feature `k` sees `Y` + the `k−1` already-generated features). But **Set 1 frozen-immutable collapses to validity 0.13 + LOF 7.9e6** (in-bounds but off-manifold), because HELOC's immutables (credit age/history) carry most of the class signal, so pinning them to the factual (wrong-class) values forces an invalid, implausible configuration. Beam-frozen still beats Exp 2 on plausibility/proximity (LOF 7.9e6 vs 3.1e9, OOB 0 vs 0.72, prox 0.46 vs 1.67) but not validity — Exp 2's 0.52 was bought via 72% OOB extrapolation. Whether Exp 4 "works" therefore depends on whether actionability is required.
