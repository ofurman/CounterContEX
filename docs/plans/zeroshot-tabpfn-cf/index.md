...
| 10 | Iterative greedy CF (sparsity-optimal) | PROMOTED | Promoted to its own standalone plan: [`iterative-greedy-cf`](../iterative-greedy-cf/index.md) (4 stages: greedy core + both selectors, selector ablation, kNN context selection, context size×strategy ablation). Builds on Stages 1–9 here. | — |

Statuses: `PENDING` -> `IN_PROGRESS` -> `DONE` | `BLOCKED` | `SKIPPED` | `PROMOTED` (moved to a standalone plan)

Phases: A = Stages 1–3 (infra), B = Stages 4–5 (experiments), C = Stage 6 (refinement), **D = Stages 7–8 (post-review remediation)**, **E = Stage 9 (feature-ordering ablation)**. Stage 4 is a **gate** for Stage 5: if single-feature reconstruction is no better than the marginal baseline, note it prominently before proceeding — Stage 5 still runs but expectations are adjusted. Stage 9 is an independent follow-up ablation; it depends only on Stages 1–8 being DONE. The iterative-greedy work originally drafted as Stage 10 here was promoted to the standalone [`iterative-greedy-cf`](../iterative-greedy-cf/index.md) plan.
