...
| 9 | [Feature-ordering (DAG) ablation](stages/09-feature-ordering-ablation.md) | PENDING | Y-first → immutables → actionable chain via explicit `dag=`; 2-factor ablation (ordering × actionable-set size). Honest framing: Y-first is a no-op in the current `impute` path, meaningful only via DAG; targets coherence, NOT HELOC's OOB root cause (reduced actionable-set is the lever for that). | — |
