...
2. Wrap lightweight baselines first.
   - Where: new `methods/nice.py` and `methods/optimization.py`, reusing `greedy_nice_counterfactual()`, `wachter_coordinate_counterfactual()`, and `growing_spheres_counterfactual()`.
...
- [ ] GATE lazy-import tests import every new method module and execute retained CLI `--help` paths without DiCE, FACE, or TabICL runtime initialization — eager dependency loading turns them red. Registry import safety is gated in Stage 6 after `methods/registry.py` exists.
