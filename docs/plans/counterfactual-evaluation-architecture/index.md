...
| Ablation identity | parameters are split across constants, CLIs, and result rows | canonical `RunSpec` expansion gives distinct stable run IDs/manifests for method, backend, hyperparameter, dataset, seed, implementation/checkpoint, and evaluation-version changes; output path, cache, device, and host changes do not change identity, and every method consumes the resolved seed | GATE | block orchestration stage | REPORT `NOT MEASURED` and block |
