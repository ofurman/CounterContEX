...
| Strict aggregation rejects any directory inside a matrix `output_root` that lacks a `COMPLETE` marker, so analysis products, logs, and launch markers must live beside the root (`<output_root>_analysis`, `campaign/launch/`), never inside it; writing them inside failed the Helios E2b job after 7 hours of finished generation. | [paper-experiment-campaign](paper-experiment-campaign/index.md) | 2026-09-07 |
