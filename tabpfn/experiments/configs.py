...
EXP2A_SCM = dict(
    ...
    prior_mlp_activations=nn.Tanh,
    perturbation_strategy="fixed_magnitude",
    perturbation_prob=1.0,  # all features perturbed (deterministic, like Exp 1)
    perturbation_direction="positive",
    fixed_magnitude_k=1.0,
    use_fixed_scm=True,
    mask_supervision=False,  # trivially all-1s mask
)
...
EXP2B_SCM = dict(
    ...
    prior_mlp_activations=nn.Tanh,
    perturbation_strategy="fixed_magnitude",
    perturbation_prob=1.0,  # all features perturbed (deterministic, like Exp 2A)
    perturbation_direction="positive",
    fixed_magnitude_k=1.0,
    use_fixed_scm=True,
    mask_supervision=False,  # trivially all-1s mask
...
# ---------- Validity Loss Variants ----------

EXP3S_VAL_CONFIG = dict(**EXP3S_CONFIG, loss_type="validity")
EXP3S_VAL_SCM = dict(EXP3S_SCM)
EXP3S_VAL_CRITERIA = dict(EXP3S_CRITERIA)

EXP4S_VAL_CONFIG = dict(**EXP4S_CONFIG, loss_type="validity")
EXP4S_VAL_SCM = dict(EXP4S_SCM)
EXP4S_VAL_CRITERIA = dict(EXP4S_CRITERIA)
...
EXPERIMENT_REGISTRY = {
    ...
    "exp3s_val": (EXP3S_VAL_CONFIG, EXP3S_VAL_SCM, EXP3S_VAL_CRITERIA),
    "exp4s_val": (EXP4S_VAL_CONFIG, EXP4S_VAL_SCM, EXP4S_VAL_CRITERIA),
}
