...
def run_dataset(
    ...
    *,
    ...
    if dataset_name not in DATASETS:
        raise ValueError(f"Unsupported Exp14 dataset: {dataset_name!r}")
    ...
    l2 = np.linalg.norm(X_cf[valid] - X_test[valid], axis=1)
    ...
    row: dict[str, Any] = {
        ...
        **common_metrics,
        ...
        "action_count_mean": (
            float(changed_actions[valid].mean()) if valid.any() else float("nan")
        ),
        "path_cost_mean": float(np.nanmean(path_costs)),
        "path_steps_mean": (
            float(path_steps[valid].mean()) if valid.any() else float("nan")
        ),
    ...
    prefix = results_dir / f"exp14_face_knn_{dataset_name}"
...
def main() -> None:
    ...
    if len(rows) > 1:
        _write_csv(args.results_dir / "exp14_face_knn_all_metrics.csv", rows)
