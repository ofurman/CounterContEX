# TabPFN - Counterfactual Generation from SCM Prior
...
| Features | 10 |
| Total samples (last batch) | 3,200 |
...
| Lowest L2 (all pairs) | gradient_guided | 1.5792 |
| Lowest L2 (valid only) | gradient_guided | 1.7163 (n=68) |
...
**Validity:**
- **uniform_random** achieves the highest validity (7.4%, 712 flipped pairs) but at the cost of the largest distances.
- **gradient_guided** has the lowest validity (0.7%, 68 pairs) -- perturbations are too small to cross decision boundaries.
- The **high_perturbation** config variant reaches **15.8% validity** (1,514 pairs), demonstrating the framework scales with perturbation intensity.
- **Shallow SCMs (2 layers)** produce **zero** label flips -- insufficient causal propagation depth.

**Proximity (ALL vs VALID):**
- Valid counterfactuals are consistently **further** from their factual points than average. The L2 valid/all ratio ranges from **1.08x** (fixed_magnitude) to **1.33x** (marginal_replacement).
- This makes sense: label flips require larger perturbations to cross the decision boundary.
- **gradient_guided** achieves the smallest gap (ratio 1.087), meaning its valid pairs are close to minimal perturbations.

**Valid-only proximity:**
- Among valid pairs, **gradient_guided** produces the most proximal counterfactuals (L2=1.72, cosine=0.999) but yields very few (68).
- **fixed_magnitude** offers the best balance: moderate validity (4.2%) with reasonable valid-pair L2 (17.6) and high cosine similarity (0.90).

**Dataset characteristics:**
- The SCM prior generates features with near-zero mean, std ~12-14, and near-symmetric distributions (skewness ~0, kurtosis ~0).
- Inter-feature correlations are moderate (mean |corr| ~0.10-0.14, max ~0.33-0.35), reflecting the causal graph structure.
- Feature-target correlations are weak (mean |corr| ~0.09-0.13), consistent with the target being a non-trivial function of upstream features.
- Class balance is perfect (0.50/0.50) due to `BalancedBinarize`.
- Deeper SCMs (6 layers) produce higher feature variance (std=19.5 vs 8.4 for 2 layers) due to cascading transformations.

**Timing:**
- All strategies run at **0.008--0.023 ms/sample** on CPU. Generation is not a bottleneck.
- gradient_guided is ~2.7x slower due to per-feature finite-difference computation.

**Sparsity:**
- Valid counterfactuals perturb slightly more features on average (~3.55 vs 3.02 for all pairs), indicating that flipping a label tends to require perturbing more dimensions.
